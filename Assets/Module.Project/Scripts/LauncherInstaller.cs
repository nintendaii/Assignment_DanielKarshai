...
namespace Module.Project {
    public class LauncherInstaller : Core.Launchers.LauncherInstaller {
        ...
        protected override void InstallCommandSignals() {
            ...
            Container.BindSignal<SignalShowMessageBox>().ToMethod<CommandMessageBox>(signal => signal.Execute).FromNew();
            
            Container.DeclareSignal<SignalHideMessageBox>();
            Container.BindSignal<SignalHideMessageBox>().ToMethod<CommandMessageBox>(signal => signal.Execute).FromNew();
        }

        protected override void InstallFactory() {
            Container.BindFactory<UnitAudioSfxController, UnitAudioSfxControllerFactory>().FromComponentInNewPrefab(unitAudioSfxControllerPrefab);
        }
    }
}